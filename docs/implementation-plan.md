...
- [x] **P0** 完整报名-缴费流程集成测试 (Section 8.1 of test-cases.md)
  - 依赖：前后端全部完成
  - 产出：`EnrollmentPaymentFlowTest.java`，覆盖 11 步端到端流程 ✅
- [ ] **P1** 访客端 UI 流程验证
  - 依赖：Phase 7 完成
...
## Phase 13: 前端管理员模块功能修复

> 详细计划见 [frontend-admin-fix-plan.md](frontend-admin-fix-plan.md)

- [ ] **P0** 批次 1：AdminLayout 全局修复（头像、搜索框）
- [ ] **P0** 批次 2：DashboardPage 修复（字段映射、View All、移除硬编码）
- [ ] **P0** 批次 3：CourseManagePage 修复（搜索/筛选/新增/编辑/分页）
- [ ] **P0** 批次 4：EnrollmentManagePage 修复（筛选/状态修改/分页）
- [ ] **P0** 批次 5：FinancePage 修复（趋势图/登记缴费/分页）
- [ ] **P1** 后端配合：新增课程分类列表 API
...
| Phase 10: 集成测试 | 3 | 1 | 33.3% |
| Phase 11: 收尾 | 5 | 0 | 0% |
| Phase 12: 代码审查修复 | 9 | 7 | 77.8% |
| Phase 13: 前端管理模块修复 | 6 | 0 | 0% |
| **合计** | **84** | **69** | **82.1%** |
